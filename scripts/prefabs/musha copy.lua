...
local function UpdateBloomStage(inst, stage)
    ...
    local silent = inst._loading or inst.components.health:IsDead() or not inst.entity:IsVisible() or
        inst.sg:HasStateTag("silentmorph")
...
local function CalcBloomRateFn(inst, level, is_blooming, fertilizer)
    ...
    local rate = (is_blooming and fertilizer > 0) and
        (season_mult * (1 + fertilizer * TUNING.WORMWOOD_FERTILIZER_RATE_MOD)) or 1
    return rate
end
...
local function guardian_proc(inst, data)
    if inst.guardian_pop and not inst.components.health:IsDead() and
        not
        (
        inst.sg:HasStateTag("rowing") or inst.sg:HasStateTag("sailing") or inst.sg:HasStateTag("sail") or
            inst.sg:HasStateTag("row") or inst.sg:HasStateTag("boating") or inst:HasTag("aquatic")) then
        ...
        if offset ~= nil then
            ...
            assasin.Transform:SetPosition(inst:GetPosition():Get())
        end
        ...
        inst:DoTaskInTime(0.5, function()
            SpawnPrefab("lightning2").Transform:SetPosition(assasin:GetPosition():Get())
...
local function guard(inst)
    if inst.components.drownable ~= nil and not inst.components.drownable:IsOverWater() then
        if not inst.no_bodyguard then
            --if not inst.no_assasin then
            inst.count_w = inst.count_w + 1
            if not inst.berserk and not inst.guard_on and inst.components.health:GetPercent() < .3 and inst.count_w >= 90
                and math.random() < .1 then
...
local function light_call(inst, data)
    ...
    for k, v in pairs(bodyguard) do
        if v.components.follower and not v.components.follower.leader and not inst.components.leader:IsFollower(v) and
            inst.components.leader:CountFollowers("bodyguard") < 1 then
    ...
    for k, v in pairs(light1) do
        if v.components.follower and not v.components.follower.leader and not inst.components.leader:IsFollower(v) and
            inst.components.leader:CountFollowers("musha_light") < 3 then
            inst.components.leader:AddFollower(v)
        end
    end
    local x, y, z = inst.Transform:GetWorldPosition()
    ...
    for k, v in pairs(light2) do
        if v.components.follower and not v.components.follower.leader and not inst.components.leader:IsFollower(v) and
            inst.components.leader:CountFollowers("musha_light2") < 1 then
            inst.components.leader:AddFollower(v)
        end
    end
end
...
local function consume_stamina(inst, data)
    --[[if inst.components.temperature:IsFreezing() or inst.components.temperature:IsOverheating() then
        if inst.components.spellpower ~= nil and inst.components.spellpower.current > 0 and not inst.components.health:IsDead() then
    ...
    if not inst.No_Sleep_Princess then
        ...
        if not (inst.tiny_sleep or inst.sleep_on) and inst.components.fatigue_sleep.current <= 100 and
        ...
        if not (inst.tiny_sleep or inst.sleep_on) and inst.components.fatigue_sleep.current <= 100 and
            inst.sg:HasStateTag("moving") and not inst:HasTag("playerghost") then
            if inst.dtired_veasy then
        ...
        if not (inst.tiny_sleep or inst.sleep_on) and inst.components.fatigue_sleep.current <= 100 and
            (
            inst.sg:HasStateTag("chopping") or inst.sg:HasStateTag("mining") or inst.sg:HasStateTag("hammering") or
                inst.sg:HasStateTag("digging") or inst.sg:HasStateTag("netting") or inst.sg:HasStateTag("fishing") or
                inst.sg:HasStateTag("read")) and not inst:HasTag("playerghost") then
            if inst.dtired_veasy then
                inst.components.fatigue_sleep:DoDelta(0.05)
            elseif inst.dtired_easy then
                inst.components.fatigue_sleep:DoDelta(0.1)
        ...
        if not (inst.tiny_sleep or inst.sleep_on) and inst.components.fatigue_sleep.current > 30 and
            not inst.sg:HasStateTag("walking") and not inst.sg:HasStateTag("running") and
            not inst.sg:HasStateTag("moving") and not inst.sg:HasStateTag("attack") and
            not inst.sg:HasStateTag("chopping") and not inst.sg:HasStateTag("mining") and
            not inst.sg:HasStateTag("hammering") and not inst.sg:HasStateTag("digging") and
            not inst.sg:HasStateTag("netting") and not inst.sg:HasStateTag("fishing") and not inst:HasTag("playerghost") then
            if inst.dtired_veasy then
                inst.components.fatigue_sleep:DoDelta(-0.2)
            elseif inst.dtired_easy then
                inst.components.fatigue_sleep:DoDelta(-0.1)
...
local function TreasureSmelling(inst, data)
    if inst.components.playercontroller then
        ...
        inst:DoTaskInTime(1, function() inst.components.playercontroller:Enable(true)
            inst.smelling = true
...
local function sleep_test(inst, data)
    ...
    local danger = FindEntity(inst, 10, function(target)
        return (target:HasTag("monster") and not target:HasTag("player") and not inst:HasTag("spiderwhisperer"))
            or
            (
            target:HasTag("monster") and not target:HasTag("player") and inst:HasTag("spiderwhisperer") and
                not target:HasTag("spider"))
    ...
    if not inst.valkyrie and not inst.berserk and not inst.sleep_on and not inst.tiny_sleep then
        ...
        if inst.components.stamina_sleep.current < 30 and inst.components.stamina_sleep.current >= 10 and
...
local function onsleepheal(inst)
    ...
    if TheWorld.state.isnight and (inst.sleep_on or inst.tiny_sleep) and inst.LightWatcher:IsInLight() and
        inst.components.hunger.current <= 0 then
        local random1 = 0.2
        local last = 1
        inst.sleep_on = false
        inst.tiny_sleep = false
    ...
    elseif TheWorld.state.isnight and (inst.sleep_on or inst.tiny_sleep) and not inst.LightWatcher:IsInLight() and
        inst.components.hunger.current > 0 then
        ...
        if TheWorld.state.isnight and (inst.sleep_on or inst.tiny_sleep) and inst.sleep_no and
            not inst.sg:HasStateTag("tent") and inst.components.hunger.current > 0 then
            local random1 = 0.2
            local last = 1
            inst.sleep_on = false
            inst.tiny_sleep = false
...
local function on_fatigue(inst, data)
    ...
    if not inst.No_Sleep_Princess then
        ...
        --dizzy
        local dizzy = 0.1
        if inst.components.stamina_sleep.current <= 0 and not inst.dizzy and math.random() < dizzy and
            not inst:HasTag("playerghost") and not inst.components.health:IsDead() and
            not (inst.sg:HasStateTag("moving") or inst.sg:HasStateTag("attack")) and not inst.sleep_on and
            not inst.tiny_sleep then
            inst.dizzy = true
        ...
        if inst.components.stamina_sleep.current > 0 and inst.components.stamina_sleep.current <= 25 and not inst.dizzy2
            and math.random() < dizzy and not inst:HasTag("playerghost") and not inst.components.health:IsDead() and
            not (inst.sg:HasStateTag("moving") or inst.sg:HasStateTag("attack")) then
            inst.dizzy2 = true
            if math.random() < dizzy then
            ...
            inst:DoTaskInTime(0, function()
                ...
                inst.sg:AddStateTag("busy")
                inst:DoTaskInTime(150, function()
                    inst.dizzy2 = false
                end)
            end)
        end
...
local function on_Critical_1(inst, data)
    ...
    if math.random() < hitcriticalchance1 and inst.components.hunger.current > 30 and not other:HasTag("structure") and
        not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall") and
        not other:HasTag("shadowminion") and other.components.health ~= nil and not other.components.health:IsDead() then
        ...
        inst:RemoveEventCallback("onhitother", on_Critical_1)
    end
end
...
local function on_Critical_2(inst, data)
    ...
    local other = data.target
    if math.random() < hitcriticalchance2 and inst.components.hunger.current > 30 and not other:HasTag("structure") and
        not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall") and
        not other:HasTag("shadowminion") and other.components.health ~= nil and not other.components.health:IsDead() then
        SpawnPrefab("explode_small").Transform:SetPosition(other:GetPosition():Get())
        other.components.health:DoDelta(-12)
        inst:RemoveEventCallback("onhitother", on_Critical_2)
    end
end
...
local function on_Critical_3(inst, data)
    ...
    local other = data.target
    if math.random() < hitcriticalchance3 and inst.components.hunger.current > 30 and not other:HasTag("structure") and
        not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall") and
        not other:HasTag("shadowminion") and other.components.health ~= nil and not other.components.health:IsDead() then
        SpawnPrefab("explode_small").Transform:SetPosition(other:GetPosition():Get())
        other.components.health:DoDelta(-16)
        inst:RemoveEventCallback("onhitother", on_Critical_3)
    end
end
...
local function on_Critical_4(inst, data)
    ...
    local other = data.target
    if math.random() < hitcriticalchance4 and inst.components.hunger.current > 30 and not other:HasTag("structure") and
        not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall") and
        not other:HasTag("shadowminion") and other.components.health ~= nil and not other.components.health:IsDead() then
        SpawnPrefab("explode_small").Transform:SetPosition(other:GetPosition():Get())
        other.components.health:DoDelta(-20)
        inst:RemoveEventCallback("onhitother", on_Critical_4)
    end
end
...
local function on_Critical_5(inst, data)
    ...
    local other = data.target
    if math.random() < hitcriticalchance5 and inst.components.hunger.current > 30 and not other:HasTag("structure") and
        not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall") and
        not other:HasTag("shadowminion") and other.components.health ~= nil and not other.components.health:IsDead() then
        SpawnPrefab("explode_small").Transform:SetPosition(other:GetPosition():Get())
        other.components.health:DoDelta(-24)
        inst:RemoveEventCallback("onhitother", on_Critical_5)
    end
end
...
local function on_Critical_6(inst, data)
    ...
    local other = data.target
    if math.random() < hitcriticalchance6 and inst.components.hunger.current > 30 and not other:HasTag("structure") and
        not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall") and
        not other:HasTag("shadowminion") and other.components.health ~= nil and not other.components.health:IsDead() then
        SpawnPrefab("explode_small").Transform:SetPosition(other:GetPosition():Get())
        other.components.health:DoDelta(-28)
        inst:RemoveEventCallback("onhitother", on_Critical_6)
    end
end
...
local function on_Critical_7(inst, data)
    ...
    local other = data.target
    if math.random() < hitcriticalchance7 and inst.components.hunger.current > 30 and not other:HasTag("structure") and
        not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall") and
        not other:HasTag("shadowminion") and other.components.health ~= nil and not other.components.health:IsDead() then
        SpawnPrefab("explode_small").Transform:SetPosition(other:GetPosition():Get())
        other.components.health:DoDelta(-32)
        inst:RemoveEventCallback("onhitother", on_Critical_7)
    end
end

local function Critical_level_1(inst)
    if (inst.valkyrie or inst.berserk) then
        inst:ListenForEvent("onhitother", on_Critical_1)
    end
end

local function Critical_level_2(inst)
    if (inst.valkyrie or inst.berserk) then
        inst:ListenForEvent("onhitother", on_Critical_2)
    end
end

local function Critical_level_3(inst)
    if (inst.valkyrie or inst.berserk) then
        inst:ListenForEvent("onhitother", on_Critical_3)
    end
end

local function Critical_level_4(inst)
    if (inst.valkyrie or inst.berserk) then
        inst:ListenForEvent("onhitother", on_Critical_4)
    end
end

local function Critical_level_5(inst)
    if (inst.valkyrie or inst.berserk) then
        inst:ListenForEvent("onhitother", on_Critical_5)
    end
end

local function Critical_level_6(inst)
    if (inst.valkyrie or inst.berserk) then
        inst:ListenForEvent("onhitother", on_Critical_6)
    end
end

local function Critical_level_7(inst)
    if (inst.valkyrie or inst.berserk) then
        inst:ListenForEvent("onhitother", on_Critical_7)
    end
end
...
local function on_Lshield_1(inst, attacked)
    ...
    if not inst.forcefields then
        if math.random() < Lshield1 and inst.components.spellpower.current >= 2 and not inst.components.health:IsDead()
            and inst.components.spellpower:GetPercent() > 0 then
...
local function on_Lshield_2(inst, attacked)
    local Lshield2 = 0.24
    if not inst.forcefields then
        if math.random() < Lshield2 and inst.components.spellpower.current >= 2 and not inst.components.health:IsDead()
            and inst.components.spellpower:GetPercent() > 0 then
            local heal_fx = SpawnPrefab("spider_heal_fx")
            heal_fx.Transform:SetScale(.4, .4, .4)
            heal_fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
...
local function on_Lshield_3(inst, attacked)
    local Lshield3 = 0.28
    if not inst.forcefields then
        if math.random() < Lshield3 and inst.components.spellpower.current >= 2 and not inst.components.health:IsDead()
            and inst.components.spellpower:GetPercent() > 0 then
            local heal_fx = SpawnPrefab("spider_heal_fx")
            heal_fx.Transform:SetScale(.4, .4, .4)
            heal_fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
...
local function on_Lshield_4(inst, attacked)
    local Lshield4 = 0.32
    if not inst.forcefields then
        if math.random() < Lshield4 and inst.components.spellpower.current >= 2 and not inst.components.health:IsDead()
            and inst.components.spellpower:GetPercent() > 0 then
            local heal_fx = SpawnPrefab("spider_heal_fx")
            heal_fx.Transform:SetScale(.4, .4, .4)
            heal_fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
...
local function on_Lshield_5(inst, attacked)
    local Lshield5 = 0.36
    if not inst.forcefields then
        if math.random() < Lshield5 and inst.components.spellpower.current >= 2 and not inst.components.health:IsDead()
            and inst.components.spellpower:GetPercent() > 0 then
            local heal_fx = SpawnPrefab("spider_heal_fx")
            heal_fx.Transform:SetScale(.4, .4, .4)
            heal_fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
...
function Call_lightining_on(inst)
    if (inst.vl1 or inst.vl2 or inst.vl3 or inst.vl4 or inst.vl5 or inst.vl6 or inst.vl7 or inst.vl8) and
        not inst.call_lightining_on then
        local x, y, z = inst.Transform:GetWorldPosition()
        local ents = TheSim:FindEntities(x, y, z, 12)
        for k, v in pairs(ents) do
            if v:IsValid() and v.entity:IsVisible() and v.components.health ~= nil and not v.components.health:IsDead()
                and not (v:HasTag("berrythief") or v:HasTag("bird") or v:HasTag("butterfly")) and
                not v:HasTag("groundspike") and not v:HasTag("player") and not v:HasTag("stalkerminion") and
                not inst.components.rider ~= nil and not inst.components.rider:IsRiding() and
                not inst.sg:HasStateTag("moving") and not inst.sg:HasStateTag("attack") and not v:HasTag("structure") and
                ...
                inst:DoTaskInTime(10, function()
                    if inst.call_lightining_on then
                        inst.call_lightining_on = false
                        inst.cast_call_lightining = false
                    end
                end)
                ...
                if not inst.cast_call_lightining then
                    ...
                    if inst.level < 430 then
                        ...
                        v:DoTaskInTime(0.3, function()
                            ...
                            if v.components.combat ~= nil and not v:HasTag("companion") then
...
local function on_hitLightnings_1(inst, data)
    inst.vl1 = false
    local other = data.target
    if other and not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall")
        and not other:HasTag("shadowminion") and other.components.health then
        inst.SoundEmitter:PlaySound("dontstarve/rain/thunder_close")
        if other and other.components.health ~= nil and not other:HasTag("structure") and not other:HasTag("alignwall") then
...
local function Lightnings_level_1(inst)
    local randomL = 0.05
    if inst.components.spellpower.current > 0 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.on_hitLightnings and math.random() < randomL and not inst.sneaka then
        inst.components.spellpower:DoDelta(-1)
        inst.on_hitLightnings = true
        inst.vl1 = true
...
local function Lightnings_level_2(inst)
    ...
    if inst.components.spellpower.current > 0 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        ...
        inst:DoTaskInTime(0, function()
            ...
            inst.components.combat:SetRange(9, 11)
            inst:DoTaskInTime(10, function() inst.on_hitLightnings = false end)
        end)
    end
end
...
local function on_sparkshield_1(attacked, data)
    if data.attacker and data.attacker.components.burnable and data.attacker.components.health and
        not data.attacker:HasTag("thorny") and not data.attacker:HasTag("shadowcreature") and
...
local function Call_lightining_attack(inst, data)
    ...
    if other and not other:HasTag("stalkerminion") and not other:HasTag("smashable") and not other:HasTag("alignwall")
        ...
        if other and math.random() < hitcall and other and other.components.health ~= nil and
            not other.components.health:IsDead() and not other:HasTag("burn") and not other:HasTag("structure") and
            not other:HasTag("groundspike") and not other:HasTag("stalkerminion") and other.components.locomotor and
            inst.components.sanity.current >= 10 then
            other:AddTag("burn")
...
local function Call_lightining_debuff(inst, data)
    ...
    for i, v in ipairs(slow) do
        ...
        if v.components.health ~= nil and not v.components.health:IsDead() and v.components.locomotor and
            not v:HasTag("wet") then
    ...
    for i, v in ipairs(slow_poison) do
        if v.components.health ~= nil and not v.components.health:IsDead() and v.components.locomotor and
            not v:HasTag("wet") and not v:HasTag("slow") then
            if v:HasTag("giant") or v:HasTag("epic") then
    ...
    for i, v in ipairs(slow_frozen) do
        if v.components.health ~= nil and not v.components.health:IsDead() and v.components.locomotor and
            not v:HasTag("wet") and not v:HasTag("slow") and not v:HasTag("slow_poison") then
            if v:HasTag("giant") or v:HasTag("epic") then
                v.components.locomotor.groundspeedmultiplier = 0.45
            else
                v.components.locomotor.groundspeedmultiplier = 0.15
...
local function On_frameshield_1(attacked, data) --
    if data.attacker and data.attacker.components.burnable and data.attacker.components.health ~= nil and
        data.attacker.components.burnable and not data.attacker:HasTag("thorny") and
        not data.attacker:HasTag("shadowcreature") and not data.attacker:HasTag("antlion") and not data.attacker.burn then
        data.attacker.components.health:DoDelta(-10)
        data.attacker.components.burnable:Ignite()
    end
    if data.attacker and data.attacker.components.burnable and data.attacker.components.burnable:IsBurning() and
        not data.attacker:HasTag("buzzard") then
...
local function on_Valkyrie_all(inst, attacked, data)
    ...
    elseif not inst.switch and weapon and weapon.components.weapon and not weapon:HasTag("electric_weapon") and
        not inst.active_valkyrie then
    ...
    if inst.on_Valkyrie_1 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_2 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(7, function() Lightnings_level_1(inst) end)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_3 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(7, function() Lightnings_level_1(inst) end)
        frameshield_1(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_4 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(6, function() Lightnings_level_2(inst) end)
        frameshield_1(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_5 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(6, function() Lightnings_level_2(inst) end)
        frameshield_2(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_6 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(5, function() Lightnings_level_3(inst) end)
        frameshield_2(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_7 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(5, function() Lightnings_level_3(inst) end)
        frameshield_3(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_8 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(5, function() Lightnings_level_4(inst) end)
        frameshield_3(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_9 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(5, function() Lightnings_level_4(inst) end)
        frameshield_4(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_10 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(5, function() Lightnings_level_5(inst) end)
        frameshield_4(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_11 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(4, function() Lightnings_level_6(inst) end)
        frameshield_4(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_12 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(4, function() Lightnings_level_7(inst) end)
        frameshield_4(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_13 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
        inst:DoTaskInTime(4, function() Lightnings_level_8(inst) end)
        frameshield_4(inst)
        inst.valkyrie_turn = true
    elseif inst.on_Valkyrie_14 and inst.active_valkyrie and inst.components.stamina_sleep.current > 0 and
        not inst.components.health:IsDead() then
...
local function Valkyrie_level_2(inst)
    inst.on_Valkyrie_2 = true
    inst.on_Valkyrie_1 = false
end

local function Valkyrie_level_3(inst)
    inst.on_Valkyrie_3 = true
    inst.on_Valkyrie_2 = false
end

local function Valkyrie_level_4(inst)
    inst.on_Valkyrie_4 = true
    inst.on_Valkyrie_3 = false
end

local function Valkyrie_level_5(inst)
    inst.on_Valkyrie_5 = true
    inst.on_Valkyrie_4 = false
end

local function Valkyrie_level_6(inst)
    inst.on_Valkyrie_6 = true
    inst.on_Valkyrie_5 = false
end

local function Valkyrie_level_7(inst)
    inst.on_Valkyrie_7 = true
    inst.on_Valkyrie_6 = false
end

local function Valkyrie_level_8(inst)
    inst.on_Valkyrie_8 = true
    inst.on_Valkyrie_7 = false
end

local function Valkyrie_level_9(inst)
    inst.on_Valkyrie_9 = true
    inst.on_Valkyrie_8 = false
end

local function Valkyrie_level_10(inst)
    inst.on_Valkyrie_10 = true
    inst.on_Valkyrie_9 = false
end

local function Valkyrie_level_11(inst)
    inst.on_Valkyrie_11 = true
    inst.on_Valkyrie_10 = false
end

local function Valkyrie_level_12(inst)
    inst.on_Valkyrie_12 = true
    inst.on_Valkyrie_11 = false
end

local function Valkyrie_level_13(inst)
    inst.on_Valkyrie_13 = true
    inst.on_Valkyrie_12 = false
end

local function Valkyrie_level_14(inst)
    inst.on_Valkyrie_14 = true
    inst.on_Valkyrie_13 = false
end
...
local function berserk_hit(inst, data)
    ...
    if inst.berserk then
        local other = data.target
        if not other:HasTag("smashable") and not other:HasTag("structure") and not other:HasTag("alignwall") and
            not other:HasTag("shadowminion") then
            ...
            if not inst:HasTag("playerghost") then
                if inst.components.hunger.current >= 160 then
                    ...
                    elseif not inst.visual_cos and not inst.change_visual then
                        if not inst.set_on and not inst.visual_hold and not inst.visual_hold2 and not inst.visual_hold3
                            and not inst.visual_hold4 then
                            inst.AnimState:SetBuild("musha")
                        elseif inst.set_on and inst.visual_hold and
                            not (inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4) then
                            inst.AnimState:SetBuild("musha")
                        elseif inst.set_on and inst.visual_hold2 and
                            not (inst.visual_hold and inst.visual_hold3 and inst.visual_hold4) then
                            inst.AnimState:SetBuild("musha_full_k")
                        elseif inst.set_on and inst.visual_hold3 and
                            not (inst.visual_hold and inst.visual_hold2 and inst.visual_hold4) then
                            inst.AnimState:SetBuild("musha_old")
                        elseif inst.set_on and inst.visual_hold4 and inst.visual_hold and inst.visual_hold2 and
                            inst.visual_hold3 then
    ...
    elseif TheWorld.state.isfullmoon and not inst.valkyrie and not inst.sleep_on and not inst.tiny_sleep then
        ...
        if not inst.berserk then
            ...
            if not inst:HasTag("playerghost") then
                ...
                elseif not inst.visual_cos and not inst.change_visual then
                    if not inst.set_on and not inst.visual_hold and not inst.visual_hold2 and not inst.visual_hold3 and
                        not inst.visual_hold4 then
                        inst.AnimState:SetBuild("musha_hunger")
                    elseif inst.set_on and inst.visual_hold and
                        not (inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4) then
                        inst.AnimState:SetBuild("musha_hunger")
                ...
                if not inst:HasTag("playerghost") and not inst.fberserk then
                    if inst.components.hunger.current >= 160 then
                        ...
                        elseif not inst.visual_cos and not inst.change_visual and not inst.musha_full then
                            if not inst.set_on and not inst.visual_hold and not inst.visual_hold2 and
                                not inst.visual_hold3 and not inst.visual_hold4 then
                                inst.AnimState:SetBuild("musha")
                            elseif inst.set_on and inst.visual_hold and
                                not (inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4) then
                                inst.AnimState:SetBuild("musha")
...
local function levelexp(inst, data)
    ...
    elseif inst.level > 4 and inst.level <= 5 then
        inst.components.talker:Say(STRINGS.MUSHA_LEVEL_LEVEL ..
    ...
    elseif inst.level > 9 and inst.level <= 10 then
        inst.components.talker:Say(STRINGS.MUSHA_LEVEL_LEVEL ..
            "3\n" ..
            STRINGS.MUSHA_T_SKILL_UNLOCK ..
            STRINGS.MUSHA_T_SKILL_V_POWER .. ": 1/4 \n" .. STRINGS.MUSHA_T_SKILL_VALKYRIE .. ": 1/2")
        inst.SoundEmitter:PlaySound("dontstarve/common/cookingpot_finish", "snd")
        inst.SoundEmitter:PlaySound("dontstarve/wilson/equip_item_gold")
        Lshield_level_1(inst)
    ...
    elseif inst.level > 6999 and inst.level <= 7000 then
        inst.components.talker:Say(STRINGS.MUSHA_LEVEL_LEVEL ..
            "30\n" ..
            STRINGS.MUSHA_T_SKILL_UNLOCK ..
            STRINGS.MUSHA_T_SKILL_DOUBLE ..
            "\n" ..
            STRINGS.MUSHA_T_SKILL_UNLOCK ..
            STRINGS.MUSHA_T_SKILL_VALKYRIE .. " \nLV 2/2\n" .. STRINGS.MUSHA_T_SKILL_UP_ACTIVE)
        inst.SoundEmitter:PlaySound("dontstarve/common/cookingpot_finish", "snd")
        inst.SoundEmitter:PlaySound("dontstarve/wilson/equip_item_gold")
        Lshield_level_5(inst)
        Critical_level_7(inst)
...
local function onkilll(inst, data)
    ...
    if IsValidVictim(victim) then
        ...
        if math.random() < largechance then
            ...
            if not inst.berserk and math.random() < 0.3 and victim:HasTag("hound") then
                local hounds = SpawnPrefab("ghosthound2")
                --inst.components.talker:Say("Come to me! Ghost puppy")
                inst:DoTaskInTime(1, function()
                    SpawnPrefab("statue_transition_2").Transform:SetPosition(victim:GetPosition():Get())
                    hounds.Transform:SetPosition(victim:GetPosition():Get())
                end)
                hounds.followdog = true --[[hounds.components.follower:SetLeader(inst)]]
            end

            if math.random() < 0.15 and (victim:HasTag("hound") or victim:HasTag("prey") or victim:HasTag("spider")) and
                victim.components.combat then
                inst:DoTaskInTime(1, function()
                    SpawnPrefab("musha_spore").Transform:SetPosition(victim:GetPosition():Get())
                end)
            end
        end
...
local function expeat(inst, food)
    ...
    elseif not --[[food.prefab == "coffe"]] string.find(tostring(food), "coffe") then
        ...
        elseif food.components.edible.hungervalue > 55 or food.components.edible.healthvalue > 29 or
            food.components.edible.sanityvalue > 19 then
            if not inst.No_Sleep_Princess and inst.components.fatigue_sleep.current >= 0 then
                inst.components.fatigue_sleep:DoDelta(-2)
                if inst.components.fatigue_sleep.current < 0 then
        ...
        elseif food.components.edible.hungervalue > 24 or food.components.edible.healthvalue > 19 or
            food.components.edible.sanityvalue > 14 then
            if not inst.No_Sleep_Princess and inst.components.fatigue_sleep.current >= 0 then
                inst.components.fatigue_sleep:DoDelta(-1)
                if inst.components.fatigue_sleep.current < 0 then
    ...
    if inst.dis_veggie_taste and not inst.berserk then
        if food.components.edible and food.components.edible.foodtype == "VEGGIE" and not food:HasTag("green_fruit") and
            food.prefab ~= "butterflywings" and food.prefab ~= "glowdust" then
            inst:DoTaskInTime(0.7, function()
                if math.random() < 0.2 then
                    inst.components.talker:Say(STRINGS.MUSHA_TALK_FOOD_BAD_1)
                elseif math.random() < 0.25 then
    ...
    -- basic taste
    if food.components.edible and not string.find(tostring(food), "monster") and not string.find(tostring(food), "human")
        and not inst.berserk then

        if inst.princess_taste then
            if food.components.edible and (food:HasTag("stale") or food:HasTag("spoiled")) and
                food.prefab ~= "butterflywings" then
                inst:DoTaskInTime(0.7, function()
                    if math.random() < 0.4 then
    ...
    if food.components.edible and string.find(tostring(food), "human") then
        if inst.princess_taste then
            inst:DoTaskInTime(1, function() inst.sg:AddStateTag("busy")
...
local function phasechange(inst, data)
    ...
    if weapon and
        (
        weapon:HasTag("range_weapon") or weapon:HasTag("bowm") or weapon:HasTag("blowdart") or weapon:HasTag("ranged") or
            weapon:HasTag("projectile") or weapon:HasTag("thrown")) then
    ...
    elseif not inst.active_valkyrie and inst.components.stamina_sleep.current >= 20 and not inst.berserks and
        not inst.fberserk then
        if inst.components.hunger.current >= 160 then
            inst.strength = "full"
            inst.active_valkyrie = false
            inst.switch = false
    ...
    if inst.sg.currentstate.name == "eat" or inst.sg.currentstate.name == "quickeat" then
        --inst.components.health.externalabsorbmodifiers:RemoveModifier(inst)
        inst.remove_shield = true
    elseif not inst.sg.currentstate.name ~= "eat" and not inst.sg.currentstate.name ~= "quickeat" then
        if inst.components.hunger ~= nil and inst.components.hunger.current > 0 then
            ...
            elseif inst.forcefields and inst.components.spellpower.current >= 5 then
                inst.components.health.externalabsorbmodifiers:SetModifier(inst, 1)
                inst.remove_shield = false
            elseif not inst.on_sparkshield and not inst.music_armor and not inst.pet_shield and not inst.active_skill and
                not inst.forcefields then
                --inst.components.health.externalabsorbmodifiers:RemoveModifier(inst)
                inst.remove_shield = true
            end
        elseif inst.components.hunger ~= nil and inst.components.hunger.current <= 0 then
            inst.forcefields = false
            --inst.components.health.externalabsorbmodifiers:RemoveModifier(inst)
            inst.remove_shield = true
        end
    ...
    if inst.remove_shield and inst.components.debuff == nil then
        inst.components.health.externalabsorbmodifiers:RemoveModifier(inst)
    elseif inst.remove_shield and inst.components.debuff ~= nil and inst.components.debuff.buff_playerabsorption then
        inst.components.health.externalabsorbmodifiers:SetModifier(inst, 0.3)
    end



    --local valkyrie = inst.valkyrie
    --inst.components.health.externalabsorbmodifiers:SetModifier(inst, .5, valkyrie)
    ...
    if not inst.full_state and inst.components.hunger.current > 180 then
        ...
        elseif not inst.princess_taste then
            if inst.normal_taste then
                inst.components.eater:SetDiet({ FOODGROUP.OMNI }, { FOODGROUP.OMNI }, { FOODTYPE.MEAT, FOODTYPE.GOODIES }
...
local function visual_hold(inst)
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and not inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.valkyrie_hold and
        inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and inst.hold_old4 and inst.hold_old5
        and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and inst.normal_k_hold and
        inst.valkyrie_k_hold and inst.berserk_k_hold then
        inst.components.talker:Say("Change Appearance\nCancel(O)key \nVisual:[Full]")
        inst:DoTaskInTime(3, function() inst.AnimState:SetBuild("musha") end)
    elseif not inst.change_visual and inst.willow and inst.wigfred and not inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and inst.hold_old4 and inst.hold_old5
        and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and inst.normal_k_hold and
        inst.valkyrie_k_hold and inst.berserk_k_hold then
        inst.components.talker:Say("Change Appearance\nCancel(O)key \nVisual:[Normal]")
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and not inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and inst.hold_old4 and inst.hold_old5
        and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and inst.normal_k_hold and
        inst.valkyrie_k_hold and inst.berserk_k_hold then
        inst.components.talker:Say("Change Appearance\nCancel(O)key \nVisual:[Valkyrie]")
        inst:DoTaskInTime(3, function() inst.AnimState:SetBuild("musha_battle") end)
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old2 and inst.hold_old3 and inst.hold_old4 and
        inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
        inst.components.talker:Say("Change Appearance\nCancel(O)key \nVisual:[Berserk]")
        inst:DoTaskInTime(3, function() inst.AnimState:SetBuild("musha_hunger") end)
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old3 and inst.hold_old4 and
        inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old4 and
        inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old6 and inst.hold_old8 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.full_k_hold and
        inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.normal_k_hold
        and inst.valkyrie_k_hold and inst.berserk_k_hold then
        inst.components.talker:Say("Change Appearance\nCancel(O)key \nVisual:[Change Appearance 8]")
        inst:DoTaskInTime(3, function() inst.AnimState:SetBuild("musha_hunger_sw2") end)
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        inst.visual_hold2 and inst.visual_hold3 and inst.visual_hold4 and inst.full_hold and inst.normal_hold and
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold
        and inst.valkyrie_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        ...
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold
        and inst.normal_k_hold and inst.berserk_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        ...
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold
        and inst.normal_k_hold and inst.valkyrie_k_hold then
    ...
    elseif not inst.change_visual and inst.willow and inst.wigfred and inst.set_on and inst.visual_hold and
        ...
        inst.valkyrie_hold and inst.berserk_hold and inst.hold_old1 and inst.hold_old2 and inst.hold_old3 and
        inst.hold_old4 and inst.hold_old5 and inst.hold_old6 and inst.hold_old7 and inst.hold_old8 and inst.full_k_hold
        and inst.normal_k_hold and inst.valkyrie_k_hold and inst.berserk_k_hold then
...
local function temp_bad_hot(inst)
    return inst.components.temperature ~= nil and inst:IsOverheating() and
        inst.components.temperature:GetCurrent() > TUNING.OVERHEAT_TEMP
end
...
local function ClientSpellpowerPercentChange(inst)
    if not TheWorld.ismastersim and inst.spellpowerbadge ~= nil then
        if inst._clientcurrentspellpowerperc ~= inst._currentspellpowerperc:value() then
            ...
            local anim_up = up and "arrow_loop_increase" or "neutral"

            local down = inst.unstable_mana
            local anim_down = down and "arrow_loop_decrease" or "neutral"


            if inst.arrowdir ~= anim_down then
                inst.arrowdir = anim_down
                inst.spellpowerarrow:GetAnimState():PlayAnimation(anim_down, true)

            elseif inst.arrowdir ~= anim_up then
                inst.arrowdir = anim_up
                inst.spellpowerarrow:GetAnimState():PlayAnimation(anim_up, true)
            end
...
local function ForceFields_on(inst, data)
...
local function master_postinit(inst)
    ...
    inst.components.foodmemory:SetDuration(360)
    inst.components.foodmemory:SetMultipliers(.45, .4, .35, .3, .2 )]]
...
inst:WatchWorldState("nighttime", function(inst) phasechange(inst) end , TheWorld)
    ...
    inst.manashield = inst:DoPeriodicTask(20, function(inst)
        if not inst.forcefields then
            if inst.field_check then
                ...
                local field_check = SpawnPrefab("field_check")
                field_check.entity:SetParent(inst.entity)
                field_check.Follower:FollowSymbol(inst.GUID, "headbase", -5, -300, -0.5)
            end
            inst.forcefields = true
        end
    end)
    ...
    inst.Call_lightining_defense = function(attacked, data)
        ...
        --[[if not inst.active_sparkshield and not inst.berserk and not inst.fberserk and not inst.pet_shield then
		inst.components.spellpower:DoDelta(-5)
		end]]
        ...
        if inst.field_check and not inst.field_check_on then
            inst.field_check_on = true
            inst:DoTaskInTime(5, function(inst)
                if inst.field_check_on then
                    inst.field_check_on = false
                end
            end)
        end


        inst.field_check = true

        --[[if not inst.forcefields then
		inst:DoTaskInTime(20,function(inst)
		inst.forcefields = true 
		
		if inst.forcefields and inst.field_check then
			inst.field_check = false
			local field_check = SpawnPrefab("field_check")
			field_check.entity:SetParent(inst.entity)
			field_check.Follower:FollowSymbol(inst.GUID, "headbase", -5, -300, -0.5)
        ...
        if inst.LightWatcher:IsInLight() and data and data.attacker --[[and data.attacker.components.burnable]] and
            not inst.fberserk and not inst.berserks and data.attacker.components.health ~= nil and
            not data.attacker.components.health:IsDead() and not data.attacker:HasTag("thorny") and
            not data.attacker:HasTag("shadowcreature") and not data.attacker:HasTag("burn") and
            not data.attacker:HasTag("groundspike") and data.attacker.components.locomotor and
            inst.components.sanity.current >= 10 then
    ...
    inst.check_sleeping = inst:DoPeriodicTask(2, function()
        ...
        if inst.components.poisonable ~= nil and inst.components.poisonable.poisonfx ~= nil then
            ...
            if inst.components.temperature ~= nil then
                if inst:IsFreezing() and inst.components.temperature:GetCurrent() < 0 then
                    inst.unstable_mana = true
                elseif inst:IsOverheating() and inst.components.temperature:GetCurrent() > TUNING.OVERHEAT_TEMP then
                    inst.unstable_mana = true
                elseif inst.components.temperature:GetCurrent() >= 0 or
                    inst.components.temperature:GetCurrent() <= TUNING.OVERHEAT_TEMP then
                    inst.unstable_mana = false
                end
            end
        end
    ...
    inst.check_status_sleep = inst:DoPeriodicTask(2, function()
		...
		inst.sleep_debuff_30 = false]]
            ...
            elseif not (inst.sleep_on and inst.tiny_sleep) and inst.components.health and
                not inst.components.health:IsDead() and not inst:HasTag("playerghost") then
                    ...
                    elseif inst.components.fatigue_sleep.current > 0 and inst.components.fatigue_sleep.current <= 30 then
                        ...
                        if not inst.tired_debuff_90 then
                            ...
                            if tired_debuff_90 then
                                ...
                                tired_debuff_90.Follower:FollowSymbol(inst.GUID, inst.components.combat.hiteffectsymbol,
                                    -10, 40, -0.5)
                            end
                        end
                    ...
                    elseif inst.components.fatigue_sleep.current > 30 and inst.components.fatigue_sleep.current <= 60 then
                        ...
                        if not inst.tired_debuff_70 then
                            ...
                            if tired_debuff_70 then
                                ...
                                tired_debuff_70.Follower:FollowSymbol(inst.GUID, inst.components.combat.hiteffectsymbol,
                                    -10, 40, -0.5)
                            end
                        end
                    ...
                    elseif inst.components.fatigue_sleep.current > 60 and inst.components.fatigue_sleep.current <= 80 then
                        ...
                        if not inst.tired_debuff_50 then
                            ...
                            if tired_debuff_50 then
                                ...
                                tired_debuff_50.Follower:FollowSymbol(inst.GUID, inst.components.combat.hiteffectsymbol,
                                    -10, 40, -0.5)
                            end
                        end
                    ...
                    elseif inst.components.fatigue_sleep.current > 80 then
                        ...
                        if not inst.tired_debuff_30 then
                            ...
                            if tired_debuff_30 then
                                ...
                                tired_debuff_30.Follower:FollowSymbol(inst.GUID, inst.components.combat.hiteffectsymbol,
                                    -10, 40, -0.5)
                            end
                        end
                    end
    ...
    inst.On_freeze = function(attacked, data)
        ...
        if inst.berserk and data.attacker and data.attacker.components.freezable and not data.attacker:HasTag("thorny")
            and not data.attacker:HasTag("shadowcreature") and not data.attacker.freezy then
    ...
    inst.on_shard_freeze = function(attacked, data)
        --local attacker = data.attacker
        local weapon = inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS)
        if data.attacker and not data.attacker:HasTag("thorny") and not data.attacker:HasTag("shadowcreature") and
            data.attacker.components.health and data.attacker.components.combat then
            ...
            if data.attacker.components.burnable and data.attacker.components.burnable:IsBurning() and
                not data.attacker:HasTag("buzzard") then
                data.attacker.components.burnable:Extinguish()
            end
        end
    ...
    inst.shield_aura = inst:DoPeriodicTask(3, function()
        if inst.on_sparkshield then
            ...
            for k, v in pairs(ents) do
                if v:IsValid() and v.entity:IsVisible() and v.components.health ~= nil and
                    not v.components.health:IsDead() and
                    not (v:HasTag("berrythief") or v:HasTag("bird") or v:HasTag("butterfly")) and
                    not v:HasTag("groundspike") and not v:HasTag("player") and not v:HasTag("companion") and
                    not v:HasTag("stalkerminion") and not v:HasTag("structure") and v.components.combat ~= nil and
                    (v.components.combat.target == inst or v:HasTag("monster") or v:HasTag("burn")) then
